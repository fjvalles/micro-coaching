...
module Whatsapp
  class Client
    ...
    def post(payload)
      ...
      max_attempts = Setting.fetch("whatsapp_retry_max") || DEFAULT_MAX_ATTEMPTS
      api_version  = Setting.fetch("meta_api_version").presence ||
      ...
      begin
        ...
        elsif retryable?(response.code.to_i) && attempt < max_attempts
          sleep(0.5 * (2**attempt))
          raise "retry"
        else
          Response.new(success?: false, raw: parsed, error: build_error_message(parsed, response.body))
        end
      rescue StandardError => e
        retry if e.message == "retry"
