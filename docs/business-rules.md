...
- **Enforce.** `app/services/participants/inbound_intent_classifier.rb:95-110`, `app/services/participants/inbound_intent_classifier.rb:151-155`, `app/services/participants/inbound_intent_classifier.rb:224-243`, `app/jobs/process_incoming_message_job.rb:105-112`, `app/jobs/process_incoming_message_job.rb:287-311`.

### 7.6 Confirmaciones simples del reto
...
- **Regla.** `Participants::IntakeStarter` pone al participante en `status: :intake`, resetea `intake_state` (jsonb: `{ "step", "answers", "awaiting_open", "generation_requested_at", "awaiting_review" }`) y envía un **template de apertura** (`SendIntakeOpenerJob`, template `intake_opener_template`, default `bienvenida_piloto`). Mientras `intake?`, `MessageClassifier` clasifica **todo** inbound como `:program_intake` (antes que cualquier otra rama). Cada respuesta la registra `Participants::IntakeHandler`, que persiste la respuesta bajo la clave de la pregunta actual, incrementa `step` y devuelve la siguiente pregunta — o completitud. Al completar, `ProcessIncomingMessageJob` marca `generation_requested_at` antes de enviar el ack/encolar generación para que mensajes posteriores ("ok", dudas, reintentos de webhook) no repitan el cierre ni creen jobs duplicados. El cuestionario vive en `Participants::IntakeQuestions::ALL` (orden = fuente de verdad del step; **solo agregar al final**, nunca reordenar mid-flight).
- **Arranque en frío (ventana 24h).** WhatsApp rechaza texto libre como primer contacto (sin ventana abierta) — Meta acepta el envío pero lo marca `failed`. Por eso el primer mensaje del intake es un **template aprobado**; la primera respuesta del participante (`awaiting_open: true`) **no se registra como respuesta** — solo abre la ventana de 24h, y `ProcessIncomingMessageJob#handle_program_intake` recién ahí envía la pregunta 1 en texto libre. A partir de la segunda respuesta, `IntakeHandler` registra normalmente.
- **Por qué.** El step numérico en `intake_state` evita depender del conteo de mensajes; `generation_requested_at` cubre el borde posterior a la última pregunta, cuando el participante sigue en `status: :intake` mientras la plantilla queda lista para revisión.
