...
- **Reanudación con gracia de 4 horas.** Al resolver manualmente un check-in tardío, el servicio llama a `DayAdvancer` y encola el despertar del nuevo día solo si la hora local está entre `wake_hour` y `wake_hour + 4h`. Antes de `wake_hour` se deja que el cron normal lo envíe; después de la gracia se espera al día siguiente para evitar entregar un desafío demasiado tarde.

## Programa de Habilidades SAESA — coaching GROW sobre el motor existente (09-jul-2026)

Un cliente (SAESA, distribución eléctrica) pidió un programa de coaching de habilidades de 7 días con modelo GROW: diagnóstico (día 1), devolución de 3 habilidades a desarrollar (día 2), tres sesiones GROW una por habilidad (días 3–5), chequeo (día 6) y cierre (día 7). El spec original venía como un system prompt que controlaba manualmente horario, avance de día y no-respuesta. Se implementó como **data pura** (`db/seeds/programs/habilidades_saesa.rb`: `Company` SAESA + `Program` company-only de 7 días + 7 `DayContent`), sin tocar el motor.

- **El prompt no re-implementa lo que el motor ya hace.** Horario 06:00–23:00 → `wake_hour`/`checkin_hour` (envío gated por código). "No avanzar sin cerrar el día" → `DayAdvancer` (avanza solo si hubo `checkin_response`). No-respuesta / reenganche / pausa → `overdue_checkin_pending?` + `PauseInactiveParticipantsJob`. Resumen final → `GenerateAndSendManifestoJob`. Meter esa lógica en el prompt habría duplicado el estado y hecho que el LLM auto-gatee tiempo (poco confiable).
- **GROW multi-turno se mapea a mañana + chat libre + check-in.** Cada día: el despertar lanza la etapa (p.ej. Goal), `FreeResponseGenerator` acompaña Reality/Options/Will en chat libre usando `DayContent#ai_system_prompt` como "Foco de hoy", y el check-in de la tarde recoge el Will/compromiso. No se creó un day-model nuevo de turnos.
- **Todo día lleva check-in, incluso los "livianos" (2 y 6).** El spec marcaba 0 preguntas el día 2; pero `DayAdvancer` exige un `checkin_response` para avanzar, así que cada día trae un check-in breve y con propósito (el del día 2 confirma cuál habilidad resuena, reforzando la continuidad).
- **El estado (3 habilidades, compromisos, respuesta1/2) viaja por `Participant#ai_summary`**, refrescado tras cada check-in por `RefreshParticipantSummaryJob`, + último reporte + historial reciente. No se agregaron columnas de estado estructurado. Los prompts de los días 3–5 degradan con gracia: si no identifican la habilidad priorizada en memoria, le preguntan a la persona en vez de inventar. **Prerrequisito del piloto: `participant_summary_enabled = true`.**
- **`response_mode: "approve"` por defecto** (cada mensaje de IA se revisa antes de enviarse), igual que el piloto de Cerebro en Marcha; se cambia a `auto` cuando el copy esté calibrado.
- **Templates reusados, sin altas en Meta.** 7 días ≤ el ciclo de 14, así que reusa `despertar_dia_01..07` / `checkin_dia_01..07` ya aprobados.
- **Rol del participante ([rol] del spec) → `focus_hint`** vía `ENV["ROLE"]` en la asignación; es el único insumo por-persona que la IA sí puede leer.
- **Gap conocido (aceptado para el piloto):** el banco `[preguntas_usadas]` (dedupe de preguntas de diagnóstico entre ciclos del mismo participante) no se persiste; en la primera corrida la lista está vacía y el prompt del día 1 igual instruye no repetir y combinar si todas se usaron.
